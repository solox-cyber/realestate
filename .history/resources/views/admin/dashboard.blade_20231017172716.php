...
<div class="content-body">
    ...
    <canvas id="chartBar"></canvas>
        ...
        <p class="mb-2 d-flex align-items-center  fs-16 text-black font-w500">{{$location->town}}
            <span class="pull-right text-dark fs-14 ms-2">{{ $location->listings_count }} Unit</span>
        </p>
        <div class="progress mb-4" style="height:10px">
            <div class="progress-bar bg-primary progress-animated" style="width:{{ ($location->listings_count/$totalListings) * 100 }}%; height:10px;" role="progressbar">
                <span class="sr-only">{{ ($location->listings_count/$totalListings) * 100 }}% Complete</span>
            </div>
        </div>
        @endforeach
    </div>
...
<script src="https://unpkg.com/leaflet@1.7.1/dist/leaflet.js"></script>
        ...
        <script>
    var months = {!! json_encode($monthlySoldData->pluck('month')->toArray()) !!};
...
<script>
    var map = L.map('world-map').setView([0, 0], 2); // Set the initial view of the map
    ...
    @foreach($locations as $location)
        L.marker([{{ 40 }}, {{ $location->longitude }}]).addTo(map)
